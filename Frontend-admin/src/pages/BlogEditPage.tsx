...
import { getPostById, updatePost, deletePost } from '@/api/blogService';
...
import type { ApiPostById, ApiPostUpdatePayload, PostTranslationItem } from '@/types/blog';

/** Single translation row in form state (id optional for new). */
...
/** Rimuove il suffisso -{locale} dallo slug se presente (es. "mio-articolo-it" → "mio-articolo"). */
function slugWithoutLocale(slug: string, locale: string): string {
  const s = (slug ?? '').trim();
...
function translationItemToRow(t: PostTranslationItem): TranslationRow {
  const locale = t.locale.trim().toLowerCase();
  return {
    id: t.id,
    locale,
    slug: slugWithoutLocale(t.slug ?? '', locale),
    title: t.title,
    content: t.content,
  };
}

function rowToPayloadItem(row: TranslationRow): ApiPostUpdatePayload['translations'][0] {
  return {
    ...(row.id ? { id: row.id } : {}),
...
export function BlogEditPage() {
  ...
  const [post, setPost] = useState<ApiPostById | null>(null);
  ...
  /** Index of the translation currently shown (language tab). */
  ...
  const loadPost = useCallback(
    async (signal?: AbortSignal) => {
      ...
      setError(null);
      try {
        const data = await getPostById(postId, signal);
        setPost(data);
        setStatus(data.status && data.status.trim() ? data.status.trim() : 'draft');
        const trs =
          data.translations.length > 0
            ? data.translations.map(translationItemToRow)
            : [{ id: '', locale: 'it', slug: '', title: '', content: '' }];
        setTranslations(trs);
        setActiveTabIndex(0);
        setDirty(false);
      } catch (e) {
        ...
        if (msg === 'UNAUTHORIZED' || msg === 'FORBIDDEN') {
          ...
          showToast('Impossibile caricare l\'articolo.', 'error');
        }
      } finally {
        setLoading(false);
      }
  ...
  // Avviso quando si chiude/ricarica la pagina con modifiche non salvate
  useEffect(() => {
  ...
  const addTranslation = useCallback(() => {
    ...
    setActiveTabIndex(translations.length); // switch to the new tab
    setDirty(true);
  }, [translations.length]);
  ...
  const firstSlugValid = useMemo(() => {
    ...
    if (!first) return true; // backend may use first translation's slug or generate
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      e.preventDefault();
      if (!canSave || !postId || !post) return;
      setSaving(true);
      try {
        const mainSlug = translations[0]?.slug?.trim() ?? '';
        const payload: ApiPostUpdatePayload = {
          slug: validateSlug(mainSlug).normalized ?? mainSlug.toLowerCase(),
          status: status.trim() || undefined,
          translations: translations.map(rowToPayloadItem),
        };
        await updatePost(postId, payload);
        showToast('Articolo aggiornato.', 'success');
        setDirty(false);
        setPost((prev) =>
          prev ? { ...prev, slug: payload.slug, status: payload.status, translations: [] } : null
        );
      } catch (e) {
        if (e instanceof Error && e.name === 'AbortError') return;
        const msg = e instanceof Error ? e.message : 'Errore';
        if (msg === 'UNAUTHORIZED' || msg === 'FORBIDDEN') {
          showToast(
            msg === 'UNAUTHORIZED'
              ? 'Sessione scaduta.'
              : 'Non hai i permessi per aggiornare.',
            'error'
          );
        } else if (msg === 'NOT_FOUND') {
          showToast('Articolo non trovato.', 'error');
          ...
          showToast(
            'Conflitto (409): uno slug è già usato. Usa slug diversi per ogni lingua e riprova.',
            'error'
          );
        } else {
    ...
    [canSave, postId, post, status, translations, showToast, navigate]
  );

  const handleDelete = useCallback(() => {
    if (!postId || !window.confirm('Eliminare questo articolo e tutte le sue traduzioni? L\'operazione non si può annullare.')) return;
    setDeleting(true);
    deletePost(postId)
      .then(() => {
  ...
  return (
    <div style={{ padding: '1.5rem 2rem', maxWidth: 900, margin: '0 auto' }}>
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            border: '1px solid var(--admin-error, #8b4513)',
            borderRadius: 6,
      ...
      <form onSubmit={handleSubmit}>
        ...
        {/* Language tabs: small buttons to switch view by language */}
        <div
          style={{
            display: 'flex',
            flexWrap: 'wrap',
